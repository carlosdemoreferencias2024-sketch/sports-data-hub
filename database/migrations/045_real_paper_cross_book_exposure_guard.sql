WITH ranked_open AS (
  SELECT
    id,
    FIRST_VALUE(id) OVER (
      PARTITION BY match_id, model_name, market_type, COALESCE(line, -9999::numeric), pick
...
SET data_state = 'DUPLICATE',
    ...
    archive_reason = COALESCE(rps.archive_reason, 'duplicate_cross_book_open_exposure'),
    last_refreshed_at = COALESCE(rps.last_refreshed_at, rps.entry_timestamp),
    updated_at = NOW(),
    raw_data = rps.raw_data || jsonb_build_object(
      'dedupe_guard', 'cross_book_open_exposure_v1',
      'duplicate_reason', 'same_match_model_market_pick_across_bookmakers'
    )
FROM ranked_open
WHERE rps.id = ranked_open.id
  AND ranked_open.exposure_rank > 1;

CREATE UNIQUE INDEX IF NOT EXISTS uq_real_paper_snapshots_open_exposure_no_book

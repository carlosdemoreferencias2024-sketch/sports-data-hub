...
const MODEL_NAME = "sports_data_hub_nba_fair_odds_v1";
...
async function verifiedHistoryFor(teamName: string, asOf: string): Promise<NbaResultObservation[]> {
  const result = await db.query(
    `
      ...
      WHERE tms.sport = 'basketball'
        AND tms.league_id = 'nba'
      ...
      LIMIT $4
    `,
    [normalizeTeamName(teamName), asOf, NBA_FAIR_ODDS_CONFIG.min_source_confidence, NBA_FAIR_ODDS_CONFIG.max_form_matches]
  );
...
export async function runNbaOwnedFairOdds(rawQuery: unknown = {}, rawBody: unknown = {}) {
      ...
      SELECT
        ...
        CASE
          WHEN COALESCE(match.raw_data->'season'->>'slug', '') = 'preseason'
            OR COALESCE(match.raw_data->'season'->>'type', '') = '1' THEN 'preseason'
          WHEN COALESCE(match.raw_data->'season'->>'slug', '') IN ('post-season', 'postseason')
            OR COALESCE(match.raw_data->'season'->>'type', '') = '3' THEN 'postseason'
          ELSE 'regular'
        END AS competition_type
      FROM v_valid_matches match
      JOIN match_competitors home_competitor ON home_competitor.match_id = match.id AND home_competitor.home_away = 'home'
      JOIN match_competitors away_competitor ON away_competitor.match_id = match.id AND away_competitor.home_away = 'away'
  ...
  const artifactSha256 = nbaFairOddsArtifactSha256();
  const configSha256 = nbaFairOddsConfigSha256();
  ...
  for (const match of matches.rows as Array<Record<string, unknown>>) {
    ...
    try {
      ...
      const modelVersionLabel = `${NBA_FAIR_ODDS_CONFIG.model_family}-${artifactSha256.slice(0, 12)}-cutoff-${model.training_cutoff_date}`;
      const modelVersion = input.apply
        ? await registerForecastModelVersion({
            ...
            artifactSha256,
            configSha256,
            featureSchemaVersion: NBA_FAIR_ODDS_CONFIG.feature_schema_version,
            notes: "NBA Elo, rest and recency-weighted score-margin model. Market odds are excluded. Historical inputs require formal SHA-256 evidence."
          })
        : null;
      const rawData = {
        ...
        model_family: NBA_FAIR_ODDS_CONFIG.model_family,
        ...
        fair_odds_method_version: NBA_FAIR_ODDS_CONFIG.fair_odds_method_version,
        ...
        projected_margin: model.projected_margin,
      ...
      if (input.apply && modelVersion) {
        ...
        const result = await db.query(
          `
            ...
            WHERE NOT EXISTS (
              SELECT 1 FROM model_quotes existing
          ...
          `,
          [
            matchId, MODEL_NAME, model.probabilities.home, model.probabilities.away,
            model.fair_odds.home, model.fair_odds.away, model.confidence,
            generatedAtIso, JSON.stringify(rawData), model.input_snapshot_sha256, artifactSha256
          ]
        );
        ...
        if (quoteId) inserted += 1;
          ...
          const existing = await db.query(
            `
              SELECT id FROM model_quotes
              WHERE match_id = $1::uuid AND model_name = $2 AND market_type = 'moneyline_2way'
      ...
      rows.push({
        ...
        output_sha256: model.output_sha256,
        market_inputs_used: false,
        status: input.apply ? (quoteId ? "NBA_FAIR_ODDS_RECORDED" : "NBA_FAIR_ODDS_REUSED") : "NBA_FAIR_ODDS_READY_DRY_RUN",
        basis: model.basis
      });
    } catch (error) {
      blocked += 1;
      rows.push({
        ...
        match: `${homeTeam} vs ${awayTeam}`,
        kickoff,
        status: "NBA_FAIR_ODDS_BLOCKED",
        reason: error instanceof Error ? error.message : String(error),
        verified_history: { home: homeForm.length, away: awayForm.length },
        required_history_per_team: NBA_FAIR_ODDS_CONFIG.min_form_matches,
        market_inputs_used: false
      });
    }
  }
  ...
  return {
    system_status: blocked > 0 && rows.length === blocked
      ? "NBA_FAIR_ODDS_BLOCKED"
      : input.apply ? "NBA_FAIR_ODDS_APPLIED" : "NBA_FAIR_ODDS_DRY_RUN",
    generated_at: generatedAtIso,

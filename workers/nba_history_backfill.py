...
def elo_transition(home_rating: float, away_rating: float, home_score: int, away_score: int, kind: str) -> tuple[float, float]:
    home_advantage = 25.0 if kind == "preseason" else 60.0
    ...
    base_k = 7.0 if kind == "preseason" else 20.0 if kind == "postseason" else 16.0
    margin_multiplier = min(2.2, 1.0 + math.log1p(abs(home_score - away_score)) / 5.0)
...
def build_rows(events: list[dict], evidence_dir: Path, captured_at: str) -> tuple[list[dict], list[dict]]:
    ...
    for event in sorted(events, key=lambda row: str(competition_for(row).get("date") or row.get("date") or "")):
        ...
        home_elo_after, away_elo_after = elo_transition(home_elo_before, away_elo_before, home_score, away_score, kind)
        ratings[home_id] = home_elo_after
        ratings[away_id] = away_elo_after
        evidence_sha, evidence_path = write_evidence(event, evidence_dir)
        match_id = f"espn-nba-history-{event_id}"
        season = str((event.get("season") or {}).get("year") or "")
        source_url = f"{SCOREBOARD_URL}?dates={played_at.year}&limit=500"
            ...
            team_stats.append({
                ...
                "raw_data": {
                    **raw_common,
                    ...
                    "score_environment": round((points_for + points_against) / 2.0, 3),
                    "elo_calculation_version": "nba_result_elo_v1",
                },
            })
...
def main() -> int:
    ...
    parser.add_argument("--minimum-season", type=int, default=2024)
    ...
    parser.add_argument("--evidence-dir", default=os.environ.get("NBA_HISTORY_EVIDENCE_DIR", "evidence/nba-history"))
    ...
    summary = {
        "system_status": "NBA_HISTORY_BACKFILL_DRY_RUN" if args.dry_run else "NBA_HISTORY_BACKFILL_APPLIED",
        "captured_at": captured_at,
...
if __name__ == "__main__":
    try:
        ...
        print(json.dumps({"system_status": "NBA_HISTORY_BACKFILL_FAILED", "error": str(exc)}), file=sys.stderr)
        raise SystemExit(1)
